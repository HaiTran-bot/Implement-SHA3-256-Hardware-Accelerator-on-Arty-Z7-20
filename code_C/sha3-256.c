...
static void pi(struct sha3_256 *ctx) {
    ...
    uint64_t temp[25] = {0}; //to avoid overwriting
    ...
    for (int i = 0; i < 25; i += 1) {
        ctx->state.words[new_x[i]] = temp[i];
    }
}
...
static void chi(struct sha3_256 *ctx) {
    ...
    for (int j = 0; j < 25; j += 5) {
        ...
        for (int i = 0; i < 5; i += 1) {
            ctx->state.words[i + j] ^= (~temp[(i + 1) % 5]) & temp[(i + 2) % 5]; //XOR 1 is NOT
        }
    }
}

static void iota(struct sha3_256 *ctx, uint8_t r) { //bj​=rc(j+7r),j=0..6. so pj = 2^j - 1 = [0, 1, 3, 7, 15, 31, 63]
    static const uint64_t constants[24] = {  //each round of r (24 rounds) need 7 bits
        0x0000000000000001, 0x0000000000008082, 0x800000000000808a, //rc run from rc(0) to rc(167) ( r=0 → t = 0..6 to r=23 → t = 161..167) 
...
static void keccak(struct sha3_256 *ctx) {
    for (int i = 0; i < SHA3_256_ROUNDS; i += 1) { //24 rounds
        theta(ctx);
        rho(ctx);
        pi(ctx);
        chi(ctx);
...
static void absorb(struct sha3_256 *ctx, uint8_t *data, uint64_t n) {
    for (uint64_t i = 0; i < n; i += 1) {
        ctx->state.bytes[ctx->absorbed++] ^= data[i]; //process byte by byte

        if (ctx->absorbed == SHA3_256_RATE) { //if the rate is full, perform keccak const = 136byte = 1088 bit
            keccak(ctx);
            ctx->absorbed = 0;
        }
    }
...
static void squeeze(struct sha3_256 *ctx, uint8_t *digest) {
    ctx->state.bytes[ctx->padpoint] ^= 0x06;   //pad 10*1
